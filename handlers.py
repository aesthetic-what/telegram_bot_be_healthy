...
async def health_answer(message: Message, state: FSMContext):
    ...
    if bp_category != "АГ 3-й степени":
        ...
        await message.answer(
            ...
            "мельканием мушек, слабостью в руке или ноге, загрудинной болью или отсутствием мочи? (Да/Нет)")
...
async def handle_ag_3_degree(message: Message, data: dict) -> str:
    ...
    if symptoms:
        ...
        warning = ("Ваш уровень артериального давления (АД) намного выше целевых значений. Рекомендуем Вам "
                   "регулярно изменять давление утром и вечером, а также вести дневник АД и частоты сердечных "
                   "сокращений. При ухудшении состояния и появлении выше описанных симптомов вызовите скорую медицинскую помощь (тел. 103, 112).")
...
async def smd(call: types.CallbackQuery, state: FSMContext):
    doctor_dict = {"yes": "Вы можете записать на прием через электронные сервисы или по номеру телефона поликлиники, к которой вы прикреплены:\n"
...
async def check_ur_health(call: types.CallbackQuery, state: FSMContext):
    ...
    age_advice = ""
    if age < 18:
        age_advice = ("Данный бот предназначен для оценки уровня артериального давления для лиц старше 18 лет. Если у вас есть жалобы то обратитесь к участковому педиатру.")
...
# Артериальная гипертензия список болезней
#
# если норма то вывести: Рекомендуемая норма артериального давления для Вас < 140\90
# убрать предложение записать на прием
# добавить картинку
#
#поменять меню выбора болезней:
# артериальная гипертензия
# list
# нет болезней
#
# если пользователь выбрал артериальная гипертензия
# есть ли у вас сопуствующие болезни?
# СХ
# ХБП
# Инсульт
#
#если выбрал что АГ есть
# если пользователь выбрал СХ, Инсульт или ИБС, то считать по норме 120-129/70-79 для возраста 18-64, норма 130-139/70-79 для всех кто сташе 65 включительно
# если ХБС, то считать по норме 130-139/70-79
# если выбрал "ничего", то считать по норме 120-129/70-79
#
# если выбрал что АГ нет:
# считать по норме < 140/90
# если давление в норме, то вывести "Ваш уровень артериального давления в норме. "
#                   "Продолжайте придерживаться принципов здорового образа жизни. Если у вас есть жалобы на состояние здоровья, рекомендуем обратиться к участковому терапевту"
# изменить вывод, если пользователь выбрал в меню запроса на запись "нет", вывести картинку
# если да, то вывести список поликлиник
# если давление больше 140/90, то вывести""Ваш уровень артериального давления (АД) выше нормы. "
#                    "Рекомендуем Вам регулярно изменять давление утром и вечером, "
#                    "а также вести дневник АД и частоты сердечных сокращений. "
#                    "Требуется назначение или коррекция лечения. "
#                    "Обратитесь за консультацией к лечащему врачу.""
# вывести запрос на запись на прием
#
# связать АГ2 и АГ3 в одну ф-ию
# сделать способ вывода кол-ва заходов
